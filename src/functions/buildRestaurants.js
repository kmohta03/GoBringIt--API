    ...
    try {
        ...
        SELECT json_agg(
            json_build_object(
                ...
                'email', r.email,
                'phone', r.phone,
                'minimumOrderAmmount', r.minimumOrderAmount,
                'deliveryFee', r.deliveryFee,
                'tags', (
                    SELECT json_agg(t.name)
                    FROM RestaurantTags rt
                    ...
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'modifierGroups', (
                                ...
                                FROM ItemModifierGroups img
                                JOIN ModifierGroups mg ON img.modifier_group_id = mg.modifier_group_id
                                WHERE img.item_id = i.item_id
                            )
                        )
                    )
                    FROM Items i
                    WHERE i.restaurant_id = r.restaurant_id AND i.is_featured = true
                ),
                'categories', (
                    SELECT json_agg(
                        json_build_object(
                            ...
                            'name', c.name,
                            'display_category', c.display_category,
                            'image', 'http://picsum.photos/150/150', -- Static image, replace as needed
                            'items', (
                                SELECT json_agg(
                                    json_build_object(
                                        'id', i.item_id,
